...
const APP=fs.readFileSync(path.join(__dirname,'behavior-tracker.html'),'utf8');
...
h1{font-family:'Luckiest Guy','Fredoka',sans-serif;font-weight:400;font-size:27pt;line-height:1.08;margin-bottom:9pt}
...
.code{font-family:'DejaVu Sans Mono',monospace;font-size:8.2pt;background:#F4F1E8;
  border:1.5pt solid var(--rule);border-radius:9pt;padding:9pt 11pt;margin-bottom:8pt;
...
.task{display:flex;gap:9pt;align-items:flex-start;margin-bottom:8pt}
.task .n{flex:none;width:27pt;height:27pt;border-radius:8pt;background:var(--blue);color:#fff;
...
`;
const pages=[];
const P=(kick,title,body)=>pages.push(`<div class="sheet"><div class="bar"></div>
${kick?`<div class="kick">${kick}</div>`:''}${title}${body}
<div class="foot"><span>Vibe Coding for Teachers &middot; Build a Behavior Tracker</span><span>Bright Scholar &middot; AI-Ready School</span></div>
<div class="barb"></div></div>`);
module.exports={css,pages,P,esc,APP};
